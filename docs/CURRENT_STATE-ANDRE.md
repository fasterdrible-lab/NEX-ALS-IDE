...
## IDEPage — badges git estilo VS Code na SFTP tree (v3.22.0)

- [x] **`gitFileMap`** — `useMemo` que constrói `Map<relativePath, {letter, color}>` a partir de `gitStatus.staged/unstaged/untracked`; cores: M=âmbar, A=verde, D=vermelho, ?=slate (`apps/web/src/pages/IDEPage.tsx`)
- [x] **`dirtyDirSet`** — `useMemo` que deriva `Set<dirtyDirPrefix>` dos paths do `gitFileMap`; detecta pastas com filhos modificados
- [x] **Tree node rendering** — `flatTree.map()` computa `relPath`, `gitInfo`, `dirtyFolder` por nó; nome do arquivo colorido; badge letra à direita; ponto âmbar em pastas sujas; auto-load git status quando tree carrega

## SquadPage — Atividade e arquivos modificados VS Code (v3.22.0)

- [x] **Tab Atividade — "Arquivos modificados"** — cada entrada: `FileCheck2` colorido por extensão (12 extensões na paleta `extColor`: ts/tsx/js/jsx/py/json/md/css/html/sql/sh/env), nome em negrito, path pai em muted, badge 'W' verde
- [x] **Tab Atividade — Stats bar** — contadores R/W/⚡/✗ em tempo real; `ActivityEntry` com `id/type/label/ts/status/durationMs/output`; ref+state pattern para evitar stale closures em loops async
- [x] **Tab Atividade — Log expandível** — cada ação com ícone tipo, label, timestamp, duração, saída (até 3000 chars) colapsável

## Squad — parseActions dois passes + regra OneDrive (v3.21.0)

- [x] **`parseActions` dois passes** — Pass 1: `\[\/ACTION\]?` aceita `[/ACTION` sem `]`; Pass 2: lookahead `(?=\[ACTION:|$)` captura blocos completamente truncados; `Set<number>` evita duplicatas; `makeBlock`/`extractParams` extraídos; `stripActions` limpa ambas as formas (`packages/core/src/squad/actions.ts`)
- [x] **`EXECUTOR_RULES` regra 9 — OneDrive** — quando `cwd` contém "OneDrive": scaffold em `AppData\Local\Temp\squad-scaffold`; `xcopy /E /Y /H /I` para destino final; apaga staging após copiar (`packages/core/src/squad/agents.ts`)
