...
9. **Squad — autonomia em melhoria contínua** — o fix SQUAD-04 (v3.51.0) adicionou contexto de tarefa nos pushes de recuperação; agentes delegados agora recebem lembrete do objetivo ao serem empurrados sem ACTION tags. Comportamento esperado: ciclos autônomos mais longos ainda podem divergir se o modelo escolher uma abordagem incorreta repetidamente — nesses casos o modo **Pipeline** oferece controle mais determinístico por fase.

## SQUAD-04 — Autonomia: push com contexto de tarefa (v3.51.0)

- [x] **`agentTaskContextRef`** — `useRef<Map<AgentName, string>>` adicionado em `SquadPage.tsx`; persiste o objetivo de cada agente durante a sessão autônoma; limpo a cada nova mensagem do usuário
- [x] **Preenchimento em 4 pontos** — `handleSend` (agente raiz), delegações `[DELEGAÇÃO]` (por `item.objective`), delegações `@mention` (por `extractTask()`), `runPipeline` (contexto curto por fase antes de cada `runAgentUntilDone`)
- [x] **`autonomousLoop` — push atualizado** — inclui `"Sua tarefa (lembre-se): ..."` (até 400 chars) quando agente não-root responde sem ACTION tags
- [x] **`runAgentUntilDone` — pushMsg atualizado** — inclui `"Sua tarefa: ..."` (até 350 chars) logo após `"⚠️ EXECUTE AGORA"`
